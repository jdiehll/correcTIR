...
* Direct emission (middle, red path): Thermal energy emitted by the canopy, typically the dominant component
...
**The GUI only needs to be installed once (following installation steps below).**
After that, the package can be run anytime by double-clicking on it as you would any other App.
...
## Example Use
Please refer to the **"Test Data"** folder for now. Keep in mind that the paths for the config files will have to be updated to your local folder structure.
